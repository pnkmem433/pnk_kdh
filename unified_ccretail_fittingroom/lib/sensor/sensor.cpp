...
#ifndef SENSOR_QUEUE_LOG
#define SENSOR_QUEUE_LOG 0
#endif
...
/*
  ...
  - 부작용: 핀 번호 저장
*/
Sensor::Sensor(int pin, uint8_t pinModeValue)
    : pin(pin), pinModeValue(pinModeValue) {}
              ...
              if (esp_timer_get_time() - lastTime >= event.delayMs * 1000ULL) {
                if (!isSend[i]) {
                  ...
                  EventQueue msg = {sensor->pin, event.event,
                                    static_cast<uint32_t>(event.delayMs), event.action};
      ...
      if (msg.action) {
        if (SENSOR_QUEUE_LOG) {
          Serial.printf("\n\n[queue]\tSensor Pin %d\tEvent: %s\tDelay: %d ms\t"
                        "Queue: %d/%d used\n",
                        msg.sensorPin,
                        msg.event == Event::Low ? "Open" : "Close",
