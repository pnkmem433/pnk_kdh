...
#ifdef ESP32
#include "esp_ota_ops.h"                   // For OTA factory reset
#endif
// ------------------------------------------------------------------------------------------
...
void setup(void) {
#ifdef ESP32
  // [수정] 롤백 방지 코드를 setup 시작하자마자 실행합니다.
...
//        Settings->last_module = Settings->fallback_module;
  ...
  // 2. [ESP8685용] MAC 주소 0000 방지 및 Hostname 설정 로직
  uint8_t mac[6];
  char mac_address[13];

  // eFuse(하드웨어)에서 직접 MAC 주소를 읽어옵니다.
  if (esp_read_mac(mac, ESP_MAC_WIFI_STA) == ESP_OK) {
    snprintf(mac_address, sizeof(mac_address), "%02X%02X%02X%02X%02X%02X", 
             mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
  } else {
    strncpy(mac_address, "UNKNOWN", 12);
  }

  // Pnks-MAC 주소로 호스트네임 주입 (사용자 설정이 %일 때만)
  if (strchr(SettingsText(SET_HOSTNAME), '%') != nullptr) {
    snprintf_P(TasmotaGlobal.hostname, sizeof(TasmotaGlobal.hostname)-1, PSTR("Pnks-%s"), mac_address);
    
    // 변경된 호스트네임을 기기 설정에 강제 저장 (AP 모드 이름 일치화)
