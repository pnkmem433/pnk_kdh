...
const uint8_t WIFI_CONFIG_SEC = 0;         // 0 keeps WiFi manager active until configured
...
{
  if ((0 == WIFI_CONFIG_SEC) && Wifi.config_type) {
    return true;
  }
...
void WifiConfig(uint8_t type)
{
  if (!Wifi.config_type) {
    ...
    Wifi.config_counter = WIFI_CONFIG_SEC;   // 0 means no auto timeout while WiFi manager is active
