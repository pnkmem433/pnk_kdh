...
def esp32_create_combined_bin(source, target, env):
    ...
    app_partition_name = env.BoardConfig().get("build.app_partition_name", "")
    ...
    if ("safeboot" not in env.subst("$BUILD_DIR/${PROGNAME}.bin")) and ("app0" == app_partition_name):
        print(Fore.YELLOW + "Skipping combined factory image for direct app0 OTA layout")
        return
