...
// OV5647 지원 해상도: 1920x1080@30fps, 1280x960@45fps, 800x1280@50fps, 800x800@50fps, 800x640@50fps
...
int app_video_open(char *dev, video_fmt_t init_fmt)
{
    ...
    ESP_LOGI(TAG, "카메라 장치 열기 시도: %s", dev);
    int fd = open(dev, O_RDONLY);
    if (fd < 0) {
        ESP_LOGE(TAG, "카메라 장치 열기 실패: errno=%d (%s)", errno, strerror(errno));
        ESP_LOGE(TAG, "카메라가 올바르게 연결되고 초기화되었는지 확인하세요");
        return -1;
    }
    ESP_LOGI(TAG, "카메라 장치 열기 성공 (fd=%d)", fd);

    if (ioctl(fd, VIDIOC_QUERYCAP, &capability)) {
        ESP_LOGE(TAG, "장치 정보 조회 실패: errno=%d (%s)", errno, strerror(errno));
        goto exit_0;
    }

    ESP_LOGI(TAG, "===  카메라 장치 정보 ===");
    ...
    forced_format.fmt.pix.width = EXAMPLE_FORCE_CAM_WIDTH;
    forced_format.fmt.pix.height = EXAMPLE_FORCE_CAM_HEIGHT;
...
static void video_stream_task(void *arg)
{
    ...
    ESP_LOGI(TAG, "비디오 스트림 태스크 시작");

    while (1) {
        esp_err_t ret = video_receive_video_frame(video_fd);
        if (ret != ESP_OK) {
            error_count++;
            if (error_count % 10 == 1) {  // 첫 에러와 10번째마다 로그
                ESP_LOGE(TAG, "프레임 수신 실패 (총 %"PRIu32"번 실패)", error_count);
            }
            vTaskDelay(pdMS_TO_TICKS(100));  // 에러 발생시 잠시 대기
            continue;
        }

        frame_count++;
        if (frame_count == 1) {
            ESP_LOGI(TAG, "첫 프레임 캡쳐 성공!");
        } else if (frame_count % 100 == 0) {
            ESP_LOGI(TAG, "프레임 캡쳐 중: %"PRIu32"개 (에러: %"PRIu32")", frame_count, error_count);
        }
        ...
        if (app_camera_video.video_task_delete) {
            ...
            ESP_LOGI(TAG, "비디오 스트림 태스크 종료 (총 %"PRIu32" 프레임)", frame_count);
