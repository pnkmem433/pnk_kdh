...
void Mqtt::begin()
{
    ...
    mqtt_cfg.port = _config.port;

    if (_config.user.length())
        mqtt_cfg.username = _config.user.c_str();

    if (_config.password.length())
        mqtt_cfg.password = _config.password.c_str();

    if (_config.useTls && _config.caCert.length())
        mqtt_cfg.cert_pem = _config.caCert.c_str();

    mqtt_cfg.disable_auto_reconnect = !_config.autoReconnect;
