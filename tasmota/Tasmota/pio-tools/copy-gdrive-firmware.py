...
def _get_gdrive_dir() -> pathlib.Path:
    ...
    if not gdrive_dir:
        raise RuntimeError("[ESP02S][설정] platformio.ini에 custom_gdrive_copy_dir 설정이 없습니다.")
...
def _sanitize_version(version_text: str) -> str:
    ...
    if not sanitized:
        raise RuntimeError("[ESP02S][버전] 버전 번호에서 숫자를 찾을 수 없습니다. 예: 24")
    return sanitized
...
def _copy_versioned_firmware(source, target, env):
    ...
    if not source_bin.is_file():
        _log("산출물", f"gzip 바이너리 파일을 찾지 못했습니다: {source_bin}", Fore.YELLOW)
        return

    _log("산출물", f"최종 gzip 바이너리 경로: {source_bin}")
