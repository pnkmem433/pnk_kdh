...
const OFFLINE_TIMEOUT_MS = Number(process.env.OFFLINE_TIMEOUT_MS || 35000);
...
function mergeExtraFields(target, payload, excludedKeys) {
  if (!payload || typeof payload !== 'object') return;

  for (const [key, value] of Object.entries(payload)) {
    if (excludedKeys.includes(key)) continue;
    if (value && typeof value === 'object' && !Array.isArray(value)) continue;
    target.extraFields[key] = value;
  }
}
  ...
  try {
    if (prefix === TOPIC_PREFIX) {
      ...
      if (leaf === 'status') {
        ...
        mergeExtraFields(plug, payload, ['state', 'webserver']);
      }
    ...
    if (prefix === TELE_PREFIX && id.startsWith('tasmota_')) {
      ...
      if (leaf === 'STATE') {
        ...
        mergeExtraFields(plug, payload, ['Time', 'Uptime', 'UptimeSec', 'Heap', 'SleepMode', 'Sleep', 'LoadAvg', 'MqttCount', 'Berry', 'POWER', 'Wifi', 'Hostname', 'IPAddress']);
      }
