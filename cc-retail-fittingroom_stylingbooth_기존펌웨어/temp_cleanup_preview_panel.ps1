...
$text = [regex]::Replace($text, '(?s)    \.factor-value\.is-preview \{.*?    \}', @'
...
$panelPattern = '(?s)<div class="mt-3 flex items-start justify-between gap-3">.*?<div class="mt-6">\s*<div class="flex items-center justify-between gap-3">'
$panelReplacement = @'
<div class="mt-3 flex items-start justify-between gap-3">
              <div>
                <div id="selectedTestCode" class="text-2xl font-black text-slate-950">1-1</div>
                <p id="selectedTestGroup" class="mt-1 text-sm font-semibold text-sky-700"></p>
              </div>
              <div class="flex flex-col items-end gap-2">
                <span id="selectedTestBadge" class="rounded-full bg-sky-100 px-3 py-1 text-xs font-semibold text-sky-700"></span>
...
'@
$text = [regex]::Replace($text, $panelPattern, $panelReplacement)
