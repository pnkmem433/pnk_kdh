...
FonkanFF701RfidReader::FonkanFF701RfidReader(HardwareSerial &serial,
    ...
    : serial(serial), baudRate(baudrate), callback(nullptr), rxPin(rxPin),
      txPin(txPin), taskHandle(nullptr) {}
...
void FonkanFF701RfidReader::startRead() {
  ...
  if (taskHandle == nullptr) {
    xTaskCreate(
        [](void *pv) {
          auto *reader = static_cast<FonkanFF701RfidReader *>(pv);
...
bool FonkanFF701RfidReader::getPowerLevelDbm(int &outDbm) {
  (void)outDbm;
  return false;
}

bool FonkanFF701RfidReader::setPowerLevelDbm(int dbm) {
  (void)dbm;
  return false;
}
