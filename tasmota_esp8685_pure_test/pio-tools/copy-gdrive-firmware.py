...
SERVER_PORT = "23"
SERVER_TARGET_DIR = "/ota/esp32/tasmota"
...
def _get_gdrive_dir() -> pathlib.Path:
    ...
    if not gdrive_dir:
        raise RuntimeError("[ESP32][??] platformio ??? custom_gdrive_copy_dir ?? ????")
...
def _run_command(command: list[str], start_message: str, success_message: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(f"[ESP32][??] ?? ??? ??????: {' '.join(command)}") from exc
